...
def image_callback(msg):
    ...
    try:
        latest_cam_frame = cv_bridge.imgmsg_to_cv2(msg, "bgr8")
    except CvBridgeError as e:
        rospy.logerr("CvBridge Error: {}".format(e)) # f-string 수정
...
def main():
    ...
    rospy.loginfo("Camera display {}.".format('enabled' if use_camera else 'disabled')) # f-string 수정
    ...
    while not rospy.is_shutdown():
        # 1. 배경 프레임 준비
        if use_camera and latest_cam_frame is not None:
            current_frame = latest_cam_frame.copy()
        else:
        ...
        if my_vehicle_gps and obstacle_gps:
            ...
            # 여기서는 차량이 항상 정북(0도)을 바라본다고 가정하고 계산합니다.
            # 실제 시스템에서는 IMU나 다른 센서로부터 Heading 값을 받아와야 합니다.
        ...
        if 'distance' in locals():
            info_text = "Dist: {:.1f}m | Risk: {}".format(distance, risk_level) # f-string 수정
        else:
